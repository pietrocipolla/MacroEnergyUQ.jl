...
- run a model repeatedly over a matrix of uncertain inputs;
- reuse one model per sample cluster and execute clusters with threads or
  distributed workers;
...
[GenX.jl template](@ref) or [MacroEnergy.jl template](@ref) for a full energy
model.
